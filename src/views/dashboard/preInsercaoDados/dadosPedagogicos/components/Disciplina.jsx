...
import DisciplinaModal from "../modais/disciplina/DisciplinaModal";
import { Link } from "react-router-dom";
import DataTable from "react-data-table-component";
import { confirmOperation } from "../../../../../context/AuthContext";


const Disciplina = memo(() => {
  const [show, setShow] = useState(false);
  const {disciplina, deleteDisciplina}=useContext(GlobalContext)
  ...
  async function handleDeleteDisciplina(id){
    const ok = await confirmOperation("Deseja eliminar esta disciplina?")
    if(ok){
      deleteDisciplina(id)
    }
 }
...
const filteredItems = disciplina.filter(
  (item) =>
    item.nomeDisciplina.toLowerCase().includes(filterText.toLowerCase()) 
);
  ...
  },
  {
    ...
    cell: (row) => (
      <div className="d-flex">        
        ...
        <Button onClick={() =>handleDeleteDisciplina(row.id)} className="btn btn-primary btn-icon btn-sm rounded-pill ms-2"  role="button">
                ...
                <div className="custom-table-effect table-responsive border rounded mt-3">
                  <Row>
                    ...
                    <DataTable
                      ...
                      paginationTotalRows={disciplina.length}
                    />
...
Disciplina.displayName = "Disciplina";
