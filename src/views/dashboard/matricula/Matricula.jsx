...
import AddEstudante from '../estudante/AddEstudante'
...
const Matricula = () => {
    ...
    const {showStep,setShowStep}=useContext(AuthContext)
    return (
        <Container>
            <Row>
                {/** 
                        ...
                        <Card>
                            <Card.Header className="d-flex justify-content-between">
                                <div className="header-title">
                                    <h4 className="card-title">Matrícula de estudante</h4>
                                </div>
                            </Card.Header>
                            <Card.Body>
                            
                                    <ul id="top-tab-list" className="p-0 row list-inline">
                                        <li className={` ${showStep === 'Image' ? ' active done' : ''} ${showStep === 'Personal' ? ' active done' : ''} ${showStep === 'Account' ? ' active done' : ''} ${showStep === 'A' ? 'active' : ''} col-lg-3 col-md-6 text-start mb-2 active`} id="account">
                                            <Link to="#">
                                        ...
                                        <li id="personal" className={`${showStep === 'Personal' ? ' active done' : ''} ${showStep === 'Image' ? ' active done' : ''} ${showStep === 'Account' ? 'active ' : ''} col-lg-3 col-md-6 mb-2 text-start`}>
                                            <Link to="#">
                                                <div className="iq-icon me-3">
                                        ...
                                        <li id="payment" className={`${showStep === 'Image' ? ' active done' : ''} ${showStep === 'Personal' ? 'active' : ''} col-lg-3 col-md-6 mb-2 text-start`}>
                                            <Link to="#">
                                                <div className="iq-icon me-3">
                                        ...
                                        <li id="confirm" className={`${showStep === 'Image' ? ' active ' : ''} col-lg-3 col-md-6 mb-2 text-start`}>
                                            <Link to="#">
                                                <div className="iq-icon me-3">
                                    ...
                                    <fieldset className={`${showStep === 'A' ? 'd-block' : 'd-none'}`}>
                                            <AddEstudante/>
                                    </fieldset>



                                    <fieldset className={`${showStep === 'Account' ? 'd-block' : 'd-none'}`}>
                                        <ConfirmarEstudante/>
                                    </fieldset>

                                    
                                    <fieldset className={`${showStep === 'Personal' ? 'd-block' : 'd-none'}`}>
                                        <h1>Pagamento</h1>
                                        <button type="button" name="next" className="btn btn-primary next action-button float-end" value="Submit" onClick={() => setShowStep('Image')} >Submit</button>
                                        <button type="button" name="previous" className="btn btn-dark previous action-button-previous float-end me-1" value="Previous" onClick={() => setShowStep('Account')} >Previous</button>
                                    </fieldset>
                                    <fieldset className={`${showStep === 'Image' ? 'd-block' : 'd-none'}`}>
